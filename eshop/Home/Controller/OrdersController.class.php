	...
	*/
	public function ajaxOrders(){
		...
		foreach ($ordersList as $list) {
			...
			foreach ($list as $val) {
				$html.='<ul class="item-list"><li class="td td-item"><div class="item-pic"><a href="javascript:void(0);" class="J_MakePoint"><img src="'.__ROOT__.'/Uploads/'.$val['sm_logo'].'" class="itempic J_ItemImg"></a></div><div class="item-info"><div class="item-basic-info"><a href="javascript:void(0);"><p>'.$val['goods_name'].'</p><p class="info-little">'.$val['goods_attr_id'].'<br/>包装：盒装 </p></a></div></div></li><li class="td td-price"><div class="item-price">'.$val['price'].'</div></li><li class="td td-number"><div class="item-number"><span>×</span>'.$val['num'].'</div></li><li class="td td-operation"><div class="item-operation"></div></li></ul>';
			}
			...
			if($val['state'] == '待发货'){
				...
				$html.='<p class="Mystatus">交易成功</p></div></li><li class="td td-change"><div class="am-btn am-btn-danger anniu"><a href="'.U("Comment/commentlist",array('id'=>$list[0][orders_id])).'" style="color:white">评价商品</a></div></li></div></div></div></div></div>';
			}
		}
